...
async def test_transaction_context_async(
    use_async_context_manager,
    use_call_async,
    enter_type,
    exit_type,
    middleware1_type,
    ...
    ctx.configure(
        on_enter_transaction_context=enter_hook_fn
        if enter_type == "sync"
        else async_enter_hook_fn,
        on_exit_transaction_context=exit_hook_fn
        if exit_type == "sync"
        else async_exit_hook_fn,
        middlewares=[
    ...
    should_raise_exception = any(
        [
            (middleware1_type == "async" or middleware2_type == "async")
            and not use_call_async,  # using call_async is required with async middleware
            middleware2_type == "sync"
            and handler_type
            == "async",  # using sync middleware with async handler is forbidden
            middleware1_type == "sync"
            and middleware2_type
            == "async",  # using sync middleware with async call_next is forbidden
        ]
    )

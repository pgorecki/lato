    ...
    **Example:**
    ...
    def call(self, func: Callable, *func_args: Any, **func_kwargs: Any) -> Any:
        ...
        if asyncio.iscoroutinefunction(func):
            raise TypeError(f"Using async function ({func}) with {self.__class__.__name__}.call() is not allowed. Use call_async() instead.")
        ...
        for m in self._middlewares[::-1]:
            ...
            if asyncio.iscoroutinefunction(m):
                raise TypeError(f"Using async middleware ({m}) with {self.__class__.__name__}.call() is not allowed. Use call_async() instead.")
    ...
    async def call_async(
        ...
        for m in self._middlewares[::-1]:
            if asyncio.iscoroutinefunction(m) and not asyncio.iscoroutinefunction(call_next):
                # async middleware is expecting an awaitable, so we need convert call_next to async
                call_next = partial(maybe_await, call_next)
            
            if not asyncio.iscoroutinefunction(m) and asyncio.iscoroutinefunction(call_next):
                # middleware is not able to retrieve call_next, as call_next is awaitable
                raise TypeError(f"Using sync middleware ({m}) with async call_next ({call_next}) is not allowed.")
            call_next = partial(m, self, call_next)
